...
トリガー表記は `file:<path>` / `cron:<schedule>` / `aws_cloudwatch_logs:<region>|<log_group>` / `aws_cloudwatch_alarms:<region>|<カンマ区切り alarm_names>` / `datadog_monitors:<site>|<カンマ区切り monitor_tags>` / `datadog_logs:<site>|<query>` / `jira_search:<base>|<jql>` / `github_workflow_runs:<owner/repo>[|branch=…][|workflows=…]` / `sentry_issues:<organization>/<project>` のいずれか。
