...
- Each runbook runs a **single Claude agent** (Agent SDK loop with file-edit / Bash tools). External SaaS (Datadog / Jira / Slack) auth env names, endpoints, and idempotency patterns are written directly in the runbook's `agent.prompt` — mihari itself carries no SaaS knowledge.
...
agent:
  prompt: |
    ...
    Auth via $DD_API_KEY/$DD_APP_KEY (Datadog) and -u $JIRA_USER:$JIRA_TOKEN (Jira).
    Tag the issue summary with $MIHARI_IDEMPOTENCY_KEY for dedup.
  allowed_tools:
    - "Bash(curl:*)"
    - "Bash(jq:*)"
YAML
