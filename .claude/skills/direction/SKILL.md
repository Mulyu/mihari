...
- 新トリガーは「mihari 側が能動的に取りに行く」形でのみ設計する（cron, file の延長線）
...
上の 5 原則に順番に当てて、最も浅い段階で結論が出るならそこで止める。

1. **5 原則のいずれかに正面から反していないか** → 反していれば即不採用、または「ランブック側で書けるか」へ振り直す
2. **既存の `bash` ステップで書けないか** → 書けるなら新ステップ種別を作らず、サンプルランブック（`runbooks/examples/`）として提示する
3. **既存トリガーの拡張で表現できないか** → 例: 新しい時刻パターンが欲しい、なら `cron` に閉じる
4. **既存サブコマンドの拡張で済まないか** → `mihari history` の絞り込み追加など
5. **新規追加が妥当か** → 上のすべてを潰した残りで、はじめて新トリガー / 新ステップ / 新サブコマンドを検討する
